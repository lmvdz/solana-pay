...
export interface ParsedMintURL {
...
export function parseMintURL(url: string): ParsedMintURL {
    ...
    let candymachineId: PublicKey | undefined;
    const candymachineIdParam = searchParams.get('candymachine-id');
    if (candymachineIdParam != null) {
        try {
            candymachineId = new PublicKey(candymachineIdParam);
        } catch (error) {
 ...
 */
export function parsePayURL(url: string): ParsedPayURL {
    if (url.length > 2048) throw new ParseURLError('length invalid');

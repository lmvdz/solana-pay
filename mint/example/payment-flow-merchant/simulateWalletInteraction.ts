...
import { createPayTransaction, parsePayURL } from '../../src';
...
export async function simulateWalletInteraction(connection: Connection, url: string) {
     ...
     */
    const { recipient, message, memo, amount, reference, label } = parsePayURL(url);
     ...
     */
    const tx = await createPayTransaction(connection, CUSTOMER_WALLET.publicKey, recipient, amount as BigNumber, {
        reference,
        memo,
    });

...
import { createPayTransaction, encodePayURL, findTransactionSignature, parsePayURL, validatePayTransactionSignature } from '../src';
...
(async function () {
    ...
    const { recipient, amount, splToken, reference, label, message, memo } = parsePayURL(originalURL);

    // Apps can encode the URL from the required and optional parameters
    const encodedURL = encodePayURL({ recipient, amount, splToken, reference, label, message, memo });
    ...
    const transaction = await createPayTransaction(connection, wallet.publicKey, recipient, amount!, {
        splToken,
        reference,
        memo,
    });
    ...
    const response = await validatePayTransactionSignature(
        connection,
        found.signature,
        recipient,
        amount!,

...
describe('parseURL', () => {
    describe('parsing', () => {
        describe('when given correct params', () => {
            it('should parse successfully', () => {
                ...
                const { recipient, amount, splToken, reference, label, message, memo } = parsePayURL(url);
    ...
    describe('errors', () => {
        it('throws an error on invalid length', () => {
            ...
            expect(() => parsePayURL(url)).toThrow('length invalid');
        });

        it('throws an error on invalid protocol', () => {
            const url = 'eth:0xffff';
            expect(() => parsePayURL(url)).toThrow('protocol invalid');
        });

        it('throws an error on invalid recepient', () => {
            const url = 'solana:0xffff';
            expect(() => parsePayURL(url)).toThrow('recipient invalid');
        });
        ...
        it.each([['1milliondollars'], [-0.1], [-100]])('throws an error on invalid amount: %p', (amount) => {
            ...
            expect(() => parsePayURL(url)).toThrow('amount invalid');
        });
        ...
        it('throws an error on invalid token', () => {
            ...
            expect(() => parsePayURL(url)).toThrow('token invalid');
        });
        ...
        it('throws an error on invalid reference', () => {
            ...
            expect(() => parsePayURL(url)).toThrow('reference invalid');
        });
    });
});
